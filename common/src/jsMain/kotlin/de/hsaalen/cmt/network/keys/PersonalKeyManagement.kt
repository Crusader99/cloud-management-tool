...
/**
 * User related personal private key management for storing the key in browsers local storage to allow restoring the
 * key after page reload or different user logged in.
 */
actual object PersonalKeyManagement {
    /**
     * Name of the item that is stored in browsers local storage.
     */
     ...
     * that is related to the user account. Usual this key is also stored in browsers local storage to allow restoring
    ...
    /**
     * Try to load the personal key from browsers local storage by the e-mail provided as [String] parameter. Will
     * return the private personal key when found otherwise null will be returned.
     */
    actual fun load(email: String): ByteArray? {
        val keyBase64 = window.localStorage[KEY_PERSONAL_CRYPTO_KEY] ?: return null
    ...
    /**
     * Store a private personal key, that is provided by the [ServerUserInfoDto] parameter, in browsers local storage
     * in combination with the related e-mail. The key has to be stored local storage to allow the user to restore the
    ...
    actual fun store(unencryptedInfo: ServerUserInfoDto): ByteArray {
        ...
        window.localStorage[KEY_PERSONAL_CRYPTO_KEY] = dataBase64
        key = data.cryptoKey
        return data.cryptoKey
    }

    /**
     * Remove the personal key from browsers local storage. This is usually used when user performs a logout. Will
    ...
    actual fun delete() {
        try {
            window.localStorage.removeItem(KEY_PERSONAL_CRYPTO_KEY)
        } finally {
